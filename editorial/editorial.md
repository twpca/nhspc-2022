...
但即使腳踏車和空位有同樣座標也是可以用同樣的做法作，例如應把同座標的物件變成有序的。
這邊就不解釋詳細的做法留給各位讀者自己研究了。
...
* 旋轉速度：若第 i, i+1 個齒輪為接合（c<sub>i</sub> = 1），轉速乘以 i 的齒數除以 i+1 的齒數，否則轉速不變
...
每張卡片的 W 值指有幾種可能而已，可以枚舉所有可能的 W<sub>x</sub> + W<sub>y</sub>，
把問題從最大化的問題變成判定性問題：

給一個中獎金額 C，目前有的彩券有沒有辦法湊到這個中獎金額？
重新整理題目的條件變成：
...
最基本的這是個三個維度的查詢問題，用三維度的資料結果(雖然記憶體上不太可能實作)，
可以理論上做到 O(log<sup>3</sup> n) 的查詢。

再來，我們可以將一個維度(例如 A) 排序，
每次固定其中一張彩券 i，尋找有沒有彩券 j 能湊出得獎金額 C。

如果從小到大枚舉 i 的話，
滿足 A<sub>i</sub> + A<sub>j</sub> ≥ C 的 j 的區域可以用雙指針來維護。
我們將這個區域的所有彩券 B, C 動態插入/刪除進資料結果就變成了二維的區間查詢。
（也就是說，目前在這個二維資料結構裡面所有的彩券都已經是滿足 A 條件限制的彩券 j 候選，只需要查詢 B, C 是否滿足限制就好）

----

從上面二維查詢的方法繼續延伸，我們可以改用一棵一維的線段樹來維護，
以 B 值作為樹的 index，並用最大的 C 值當成每個節點的 value。
每次決定彩券 i 後，可以算出合法的 B 的區間 B<sub>j</sub> ≥ C - B<sub>i</sub>。

改成查詢這個 B 區間最大的 C 值並檢查是某和 C<sub>i</sub> 相加滿足條件，可以再減少一個維度，
變成每次查詢 O(log n)。即是本題滿分的預設做法，複雜度 O(n log n)。
